...
func ByFD(src, dst *os.File) (bool, error) {
	if _, _, err := syscall.Syscall(syscall.SYS_IOCTL, dst.Fd(), BtrfsIocClone, src.Fd()); err != 0 {
		return false, err
	}
	return false, nil
}

func ByPath(dst, src string) (bool, error) {
